...
namespace our {    void RaceSystem::enter(Application* app) {
        this->app = app;
    ...
    void RaceSystem::checkPlayerProgress(Entity* player, RacePlayerComponent* racePlayer) {
        if (racePlayer->nextCheckpoint >= checkpoints.size()) return;
        ...
        if (distance <= checkpoint->radius) {
            ...
            if (checkpoint->isFinishLine && racePlayer->nextCheckpoint == 1) {
                ...
                if (racePlayer->raceTime > 1.0f) { // Prevent immediate lap completion at race start
                    // Record lap time
                    ...
                    if (racePlayer->currentLap > racePlayer->maxLaps) {
                        ...
                        for (size_t i = 0; i < racePlayer->lapTimes.size(); ++i) {
                            ...
                            if (i < racePlayer->lapTimes.size() - 1) std::cout << ", ";
                        }
                        std::cout << std::endl;
                    } else {
                ...
                if (racePlayer->nextCheckpoint >= checkpoints.size()) {
    ...
    float RaceSystem::calculateDistanceToNextCheckpoint(Entity* player, RacePlayerComponent* racePlayer) {
        if (racePlayer->nextCheckpoint >= checkpoints.size()) return 0.0f;
    ...
    std::string RaceSystem::getRaceStateString() const {
        if (!raceManager) return "No Race Manager";
        auto manager = raceManager->getComponent<RaceManagerComponent>();
        if (!manager) return "No Race Manager";
